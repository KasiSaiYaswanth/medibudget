...
const CostResults = ({ result, onReset }: Props) => {
  ...
  const handleDownload = () => {
    ...
    let report = `════════════════════════════════════════════════\n`;
    report += `           MEDIBUDGET COST ESTIMATION REPORT\n`;
    report += `════════════════════════════════════════════════\n\n`;
    report += `Date: ${dateStr}  |  Time: ${timeStr}\n\n`;
    report += `─── PATIENT DETAILS ───\n`;
    report += `Condition     : ${result.condition}\n`;
    report += `City          : ${result.city}\n`;
    report += `Hospital Type : ${result.hospitalType}\n\n`;
    report += `─── COST BREAKDOWN ───\n`;
    report += `Consultation  : ${formatCurrency(result.consultation)}\n`;
    report += `Tests         : ${formatCurrency(result.tests)}\n`;
    report += `Medicines     : ${formatCurrency(result.medicines)}\n`;
    report += `Treatment     : ${formatCurrency(result.treatment)}\n`;
    report += `────────────────────────\n`;
    report += `TOTAL ESTIMATED COST : ${formatCurrency(result.total)}\n\n`;
    report += `City Multiplier     : ${result.cityMultiplier}x\n`;
    report += `Hospital Multiplier : ${result.hospitalMultiplier}x\n\n`;

    if (result.recommendedTests?.length) {
      report += `─── RECOMMENDED TESTS ───\n`;
      result.recommendedTests.forEach((t, i) => {
        report += `  ${i + 1}. ${t}\n`;
      });
      report += `\n`;
    }

    if (result.recommendedMedicines?.length) {
      report += `─── MEDICINES THAT MAY BE PRESCRIBED ───\n`;
      result.recommendedMedicines.forEach((m, i) => {
        report += `  ${i + 1}. ${m}\n`;
      });
      report += `\n`;
    }
    ...
    report += `════════════════════════════════════════════════\n`;
    report += `⚕️ DISCLAIMER: This is an estimate for reference\n`;
    report += `only. Actual costs may vary. Always consult a\n`;
    report += `qualified healthcare professional.\n`;
    report += `════════════════════════════════════════════════\n`;
    report += `Generated by MediBudget | medibudget.app\n`;

    const blob = new Blob([report], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `MediBudget_Report_${result.condition.replace(/[^a-zA-Z0-9]/g, "_")}_${now.toISOString().slice(0, 10)}.txt`;
